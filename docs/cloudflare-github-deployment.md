...
4. `.github/workflows/daily-dashboard.yml` 在北京时间每周一至周五07:05运行主任务，07:45运行幂等备份检查，08:15执行过期监控与自动补跑，16:30强制运行收盘刷新；采集器会再次检查A股交易日，节假日不改写网站或产生过期告警。
5. 工作流写入新的JSON快照和Markdown归档并提交。该提交会触发Cloudflare自动部署，因此不需要在GitHub保存Cloudflare API Token。
6. 08:15监控发现交易日快照仍为旧日期时，会创建GitHub Issue告警；补跑提交成功后自动留言并关闭，失败时保留Issue供排查。
6. 在Cloudflare Worker的“域和路由”中，把已注册域名绑定到这个Worker。

## 关键行为

- 07:05主任务运行时，A股部分自动使用上一已完成交易日；海外部分使用实际采集时已发布的最新有效数据，目标在07:30前完成网站更新。
